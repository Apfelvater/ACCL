...
TEST_F(ACCLTest, eval_loop_recv_only) {
  ...
  for (int i = 0; i < loop_count; i++) {
    ...
    if (::rank == 0) {
      ...
      duration = accl->get_duration(handle);
      accl->wait(handle);
      std::cout << "Loop" << i << ": Send (R0) took " << duration << " ns" << std::endl;
    } else {
      // Waiting for 2 seconds (until the message hopefully arrived.)
      std::this_thread::sleep_for(std::chrono::milliseconds(2000));
      auto handle = accl->recv(*op_buf, count, 0, 0, GLOBAL_COMM, true, dataType::none, true);
      duration = accl->get_duration(handle);
      accl->wait(handle);
      std::cout << "Loop" << i << ": Recv (R1) took " << duration << " ns AFTER WAITING 2 SECONDS." << std::endl;
    }
  }
...
TEST_F(ACCLTest, eval_loop_send_recv) {
  int loop_count = EVAL_LOOP_COUNT;
  unsigned int count = options.count;
  ...
  for (int i = 0; i < loop_count; i++) {
    ...
    if (::rank == 0) {
      ...
      duration = accl->get_duration(handle);
      accl->wait(handle);
      std::cout << "Loop" << i << ": Recv (R1) took " << duration << " ns" << std::endl;
    }
  }
...
TEST_F(ACCLTest, eval_loop_scatter) {
  ...
  if (::rank != root) {
    for (unsigned int i = 0; i < count; ++i) {
      EXPECT_FLOAT_EQ((*res_buf)[i], (*op_buf)[count * ::rank + i]);
    }
  } else {
    EXPECT_TRUE(true);
  }
...
TEST_F(ACCLTest, eval_loop_broadcast_sync_outside) {
  ...
  for (int i = 0; i < loop_count; i++) {
    ...
    if (::rank == root) {
      ...
      if (i == loop_count - 1) sync_it = true;
      uint64_t duration = accl->eval_collective(handle, *res_buf, sync_it);
      std::cout << "Rank no. " << ::rank << " took " << duration << " to finish." << std::endl; 
